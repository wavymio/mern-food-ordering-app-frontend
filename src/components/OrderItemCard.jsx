        ...
        <Card>
            <CardHeader>
                <CardTitle className="grid md:grid-cols-4 gap-4 justify-between mb-3">
                    ...
                    <div>
                        Total Cost: 
                        <span className='ml-2 font-normal'>${(order.totalAmount/1).toFixed(2)}</span>
                    </div>
                </CardTitle>
                <Separator />
            </CardHeader>
            <CardContent className="flex flex-col gap-6">
                <div className='flex flex-col gap-2'>
                    {order.cartItems.map((cartItem, index)=>(
                        <span key={index}>
                            <Badge variant={"outline"} className="mr-2">{cartItem.quantity}</Badge> {cartItem.name}
                        </span>
                    ))}
                </div>
                <div className='flex flex-col space-y-1.5'>
                    <Label htmlFor="status">What is the status of this order</Label>
                    <Select value={status} disabled={isLoading} onValueChange={(value) => handleStatusChange(value)}>
